...
def hot_key():
    print(
        f"Hot key:\n{Fore.YELLOW + h1 + Style.RESET_ALL } =>",
        Fore.GREEN
        + "ON"
        + Style.RESET_ALL
        + "/"
    ...
    print(
        f"{Fore.YELLOW + h2 + Style.RESET_ALL } =>",
        Fore.GREEN
        + "ON"
        + Style.RESET_ALL
        + "/"
    ...
    print(
        f"{Fore.YELLOW + h3 + Style.RESET_ALL } =>",
...
def change_clipboard_text():
    ...
    while True:
        ...
        if (
            up == 1
            and current_clipboard != previous_clipboard
            and len(current_clipboard) > 0
        ):
            ...
            for char in rev_letter_mapping:
                print(
                    f"'{Fore.GREEN + char + Style.RESET_ALL}' : '{Fore.CYAN + rev_letter_mapping[char] + Style.RESET_ALL}'"
                )
            print()
            calibration_button.configure(bg="white")
            ...
            and switch_value.get()
            and not rswitch_value.get()
        ):
            current_clipboard = current_clipboard.lower()
            if len(current_clipboard) > 0:
                print(
                    Fore.BLUE
                    + f"New text(Reverse"
                    + Fore.RED
